import { W as p } from "./index-C-OVggYW.js";
class f extends p {
  constructor() {
  ...
  async migrate() {
    ...
    const t = [],
      s = [],
      n = "_cap_",
      o = Object.keys(this.impl).filter((i) => i.indexOf(n) === 0);
    for (const i of o) {
      const r = i.substring(n.length),
        a = (e = this.impl.getItem(i)) !== null && e !== void 0 ? e : "",
        { value: l } = await this.get({ key: r });
      typeof l == "string"
        ? s.push(r)
        : (await this.set({ key: r, value: a }), t.push(r));
    }
    return { migrated: t, existing: s };
  }
  async removeOld() {
    const e = "_cap_",
      t = Object.keys(this.impl).filter((s) => s.indexOf(e) === 0);
    for (const s of t) this.impl.removeItem(s);
  }
  get impl() {
